using UnityEngine;

public class InventoryTester : MonoBehaviour
{
    public Inventory inventory;
    public Barn barn;
    public InventoryUI inventoryUI1;
    public InventoryUI inventoryUI2;
    public BarnUI barnUI;
    public ItemData item1, item2, item3, item4, item5;
    public void OnClickSpawnItem()
    {
        if (inventory.AddItem(item1, 10) && inventory.AddItem(item2, 10) && inventory.AddItem(item2, 10) && inventory.AddItem(item3, 0) && inventory.AddItem(item4, 2))
        {
            inventoryUI1.UpdateAllSlots();
            
        }

        if (inventory.AddItem(item5, 3)) inventoryUI2.UpdateAllSlots();

        else
        {
            Notification.Instance.ShowNotification("Túi đồ của bạn đã đầy!");
        }
    }
}

